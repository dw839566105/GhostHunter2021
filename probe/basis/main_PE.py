...
PE = np.zeros((init_x.shape[0], PMT_bins[1:].shape[0]))
...
for j_index, j in enumerate(np.unique(EventId)):
    print(j)
    PE_single,_ = np.histogram(PMTId[EventId==j], bins=PMT_bins)

    PE[j_index] = PE_single
...
print(PE.shape, cos_theta.shape, r.shape)
