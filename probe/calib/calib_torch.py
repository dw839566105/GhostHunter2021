...
h1.close()
h2 = tables.open_file('./coeff_Time.h5')
Time_coeff = h2.root.coeff[:]
h2.close()
coeff = np.zeros((PE_coeff.shape[0] * Time_coeff.shape[0]))
for index1 in np.arange(PE_coeff.shape[0]):
    print(index1)
    for index2 in np.arange(Time_coeff.shape[0]):
        if index1 == 0:
...
for epoch in range(n_epochs):
    ...
    for index, (x_batch, y_batch) in enumerate(train_loader):
        ...
        with h5py.File('coeff_torch.h5', 'w') as out:
            out.create_dataset('coeff', data = coeff_save)
    scheduler.step()
